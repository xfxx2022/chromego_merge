...
endpointyx(){    
    # 下载优选工具软件，感谢某匿名网友的分享的优选工具
     wget https://pan.rappit.site/d/shell/warp-endpoint/warp-linux-$(archAffix) -O warp
    ...
    echo ""
    yellow "使用方法如下："
    yellow "将 WireGuard 节点的默认的 Endpoint IP：engage.cloudflareclient.com:2408 替换成本地网络最优的 Endpoint IP"

    # 删除 WARP Endpoint IP 优选工具及其附属文件
    rm -f ip.txt result.csv warp
}
...
menu(){
    clear
    echo "##########################################################"
    echo -e "# ${GREEN}ViRb3/wgcf GitHub仓库${PLAIN}: https://github.com/ViRb3/wgcf   #"
    echo -e "# ${GREEN}Warp+ Bot${PLAIN}: https://t.me/generatewarpplusbot          #"
    echo "##########################################################"
    echo ""
